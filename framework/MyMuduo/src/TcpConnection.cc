...
void TcpConnection::send(const std::string &buf)
{
    ...
    {
        if (loop_->isInLoopThread()) // 在自己的线程中发送
        {
...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    {
        nwrote = ::write(channel_->fd(), data, len); // 不一定能发送完
        if (nwrote >= 0)
        {
            remaining = len - nwrote;
            // 发送完了，不用再给channel设置写事件
...
void TcpConnection::handleWrite()
{
    if (channel_->isWriting())
    {
        ...
        if (n > 0)
        {
            ...
            if (outputBuffer_.readableBytes() == 0) // 把写缓冲的数据全部写到fd（内核）缓存区中了
            {
...
// poller 返回就绪事件 => channel::closeCallback => TcpConnection::handleClose
